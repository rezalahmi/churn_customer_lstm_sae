...
class lstm_sae:
    ...
    def __init__(self, node_layer1, node_layer2, node_layer3):
        self.epoch = 50
        timesteps = 1  # number of sample
        n_features = 97  # number of features
        self.model = keras.models.Sequential()
        self.model.add(keras.layers.LSTM(node_layer1, activation='relu', input_shape=(timesteps, n_features), return_sequences=True))
    ...
    def fit(self, X):
        self.model.fit(X, X, epochs=self.epoch, verbose=2)

    def predict(self, seq_in):
        return self.model.predict(seq_in, verbose=2)
